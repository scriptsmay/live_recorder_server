...
class DanmakuBurnQueue {
  ...
  async processTask(task) {
    const { recordingFileId, inputPath, assPath, outputPath, force = false, useQsv = false } = task;

    try {
      // 标记为处理中
      await pool.query(
        `UPDATE danmaku_burn_records SET status = 'processing', started_at = NOW() WHERE recording_file_id = $1`,
        [recordingFileId]
      );
      ...
      if (result.success) {
        ...
        await pool.query(
          `UPDATE danmaku_burn_records
           SET status = 'completed', output_path = $1, completed_at = NOW()
           WHERE recording_file_id = $2`,
  ...
  async enqueueSession(params) {
    ...
    try {
      ...
      console.log(`[弹幕压制队列] 会话 ${sessionId} 共 ${enqueued} 个分段入队`);
      return enqueued;
    } catch (err) {
    ...
    try {
      await pool.query(
        `INSERT INTO danmaku_burn_records
         ...
         ON CONFLICT (recording_file_id) DO UPDATE SET status = 'queued', enqueued_at = NOW()`,
        [
          taskData.sessionId,

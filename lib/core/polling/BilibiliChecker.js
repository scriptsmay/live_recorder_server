...
class BilibiliChecker extends PlatformChecker {
  ...
  async getStreamUrl(roomId) {
    ...
    const data = await PlatformChecker.fetchJson(`${BILIBILI_API_BASE}/room/v1/Room/playUrl?${params.toString()}`, {
      headers: {
        Origin: 'https://live.bilibili.com',
        Referer: `https://live.bilibili.com/${roomId}`,
      },
  ...
  async checkStatus() {
    ...
    try {
      ...
      const [anchorData, titleData] = await Promise.all([this.getAnchorInfo(uid), this.getRoomTitle(realRoomId)]);

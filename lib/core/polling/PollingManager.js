...
class PollingManager {
  ...
  async _extractStreamUrl(room_url, checkResult) {
    ...
    if (checkResult.streamInfo) {
      ...
      if (detectedPlatform === 'huya') {
        ...
        const status = await huyaChecker.checkStatus();
    ...
    try {
      ...
      if (isLive && currentStatus !== 'recording') {
        ...
        if (lastRecordingEnd) {
          ...
          if (now - endTime < 5000) {
            console.log(
              `[PollingManager] 检测到录制刚结束(${now - endTime}ms前)，立即重试获取新流地址: ${room.room_name || room_url}`
            );
  ...
  async _retryRecordingOnFailureWithDelay(room, retryCount) {
    ...
    try {
      ...
      if (!retryResult.error) {
        console.log(
          `[PollingManager] ✅ 重试${retryCount + 1}次录制成功: ${room_name || room_url}, 会话ID: ${retryResult.sessionId}`
        );
      } else {
        console.error(
          `[PollingManager] ❌ 重试${retryCount + 1}次录制失败: [${retryResult.code}] ${retryResult.message}`
        );
        await this._retryRecordingOnFailureWithDelay(room, retryCount + 1);
      }
    } catch (err) {

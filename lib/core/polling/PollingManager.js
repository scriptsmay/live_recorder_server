...
class PollingManager {
  ...
  _extractHuyaStreamUrl(streamInfo) {
    ...
    try {
      const sorted = [...streamInfo.streams].sort((a, b) => (b.iWebPriorityRate || 0) - (a.iWebPriorityRate || 0));

      for (const stream of sorted) {
        const priority = stream.iWebPriorityRate;
        if (priority < 0) continue;

        const streamName = stream.sStreamName;
        const suffix = stream.sFlvUrlSuffix;
        const antiCode = stream.sFlvAntiCode;
        const baseUrl = (stream.sFlvUrl || '').replace('http://', 'https://');

        if (baseUrl && streamName && suffix && antiCode) {
          // 去掉 -imgplus 后缀，走非图片增强流 CDN 节点
          // 移动端 anticode 的 ctype=tars_mp 会导致桌面 ffmpeg 被 CDN 断开
          const cleanStreamName = streamName.replace('-imgplus', '');
          const fullUrl = `${baseUrl}/${cleanStreamName}.${suffix}?${antiCode}`;
          console.log(`[PollingManager] 构建虎牙流地址: ${fullUrl.slice(0, 120)}...`);
          return fullUrl;
        }
      }

      console.warn('[PollingManager] 虎牙流信息不完整，无法构建流地址');
      return null;
    } catch (err) {
    ...
    try {
      const result = await pool.query(`
        SELECT id, room_url, room_name, polling_enabled, polling_platform,
               ...
               monitoring_enabled, notification_enabled, status
        FROM rooms

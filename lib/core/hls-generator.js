...
class HLSGenerator {
  ...
  async generate(inputPath, outputDir, sessionId = null) {
    return new Promise((resolve) => {
      ...
      const procLog = createProcLog('hls', sessionId);
  ...
  async generateForRecording(recordingId, recordingType = 'recording') {
    try {
      const table = recordingType === 'recording' ? 'recordings' : 'recording_files';
      const result = await pool.query(
        `SELECT file_path, is_hls_ready, hls_playlist_path FROM ${table} WHERE id = $1`,
        [recordingId]
      );
      ...
      const genResult = await this.generate(inputPath, outputDir);

      if (genResult.success) {
        await this.updateRecordingHLSStatus(recordingId, recordingType, genResult.playlistPath);
      }
  ...
  async updateRecordingHLSStatus(recordingId, recordingType, playlistPath) {
    try {
      const table = recordingType === 'recording' ? 'recordings' : 'recording_files';
      await pool.query(
        `UPDATE ${table} SET is_hls_ready = TRUE, hls_playlist_path = $1, hls_generated_at = NOW() WHERE id = $2`,
  ...
  async cleanupOldFiles(days = 30) {
    try {
      ...
      const result = await pool.query(
        `SELECT id, hls_playlist_path, hls_generated_at FROM recordings WHERE is_hls_ready = TRUE AND hls_generated_at < $1`,
        [cutoffDate]
      );
      ...
      let deletedCount = 0;
      for (const row of [...result.rows, ...result2.rows]) {

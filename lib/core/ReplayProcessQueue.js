...
class ReplayProcessQueue {
  ...
  async processTask(task) {
    ...
    try {
      ...
      await this.runAction(record, task.action || 'all', { logStream });
  ...
  async runAction(record, action, options = {}) {
    ...
    for (const step of actions) {
      ...
      if (step === 'extract') {
        ...
        const result = await videoProcessor.download(current, { logStream });
        if (!result.success) throw new Error(result.error);
        ...
        const result = await videoProcessor.cut(current, { logStream });
        if (!result.success) throw new Error(result.error);
        ...
        const result = await videoProcessor.fix(current, { logStream });
        if (!result.success) throw new Error(result.error);

  ...
  try {
    ...
    for (const session of sessions) {
      ...
      for (const f of uniqueFiles) {
        ...
        await pool.query(
          `INSERT INTO recording_files (session_id, room_url, file_path, file_name, file_size, status, started_at, ended_at, segment_index, segment_start_ms, segment_end_ms, checked_at)
...
async function checkSessionAss() {
  try {
    ...
    // 查询已结束会话中缺少分段 ASS 的录制文件
    const { rows: files } = await pool.query(
      `SELECT rf.id, rf.file_path, rf.session_id, rf.segment_start_ms, rf.segment_end_ms,
              rs.output_dir
       FROM recording_files rf
       JOIN recording_sessions rs ON rf.session_id = rs.id
       WHERE rs.status IN ('completed', 'interrupted')
         AND (rf.danmaku_ass_path IS NULL OR rf.danmaku_ass_path = '')
         AND rf.status = 'completed'
         AND rf.file_path IS NOT NULL
         AND rf.segment_start_ms > 0`
    );
    ...
    for (const [sessionId, sessionFiles] of sessionMap) {
      ...
      try {
        ...
        if (!fs.existsSync(jsonlPath)) {
          console.log(`[看门狗][ASS] 会话 ${sessionId} 无 danmaku.jsonl，跳过`);
          continue;
        }

        // 查询所有需要补 ASS 的分段（含本轮未命中文件列表的、之前遗漏的）
        const segments = await pool.query(
          `SELECT id, segment_index, segment_start_ms, segment_end_ms
           FROM recording_files
           WHERE session_id = $1
             AND status = 'completed'
             AND (danmaku_ass_path IS NULL OR danmaku_ass_path = '')
             AND segment_start_ms > 0
           ORDER BY id ASC`,
          [sessionId]
        );

        if (segments.rows.length === 0) continue;

        // 过滤掉已被其他流程填充的记录（double-check 竞态保护）
        const pendingSegments = [];
        for (const seg of segments.rows) {
          const recheck = await pool.query(
            `SELECT danmaku_ass_path FROM recording_files WHERE id = $1`,
            [seg.id]
          );
          const existing = recheck.rows[0]?.danmaku_ass_path;
          if (existing && existing.length > 0 && fs.existsSync(existing)) {
            continue;
          }
        ...
        for (const seg of segResults) {
          ...
          await pool.query(
            `UPDATE recording_files SET danmaku_ass_path = $1 WHERE id = $2`,
            [seg.assPath, seg.id]
          );
          updated++;
        }
        ...
        if (updated > 0) {
          console.log(
            `[看门狗][ASS] 会话 ${sessionId}: 补充生成 ${updated} 个分段的 ASS 字幕`
          );
        }
      } catch (err) {
...
async function backfillSegmentTimes(sessionId, pool) {
  try {
    ...
    console.log(`[backfillSegmentTimes] 会话 ${sessionId} 已补充分段时间，共 ${rows.rows.length} 个分段`);
  } catch (err) {

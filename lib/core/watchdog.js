...
const RECENTLY_ENDED_MS = 30000; // 刚结束会话的稳定时间：30秒（录制结束后文件已停止写入）
  ...
  try {
    ...
    for (const room of rooms) {
      ...
      if (room.output_path) {
        ...
        const isSegmented = (room.segment_duration || 0) > 0 && downloader.isSegment();
    ...
    for (const room of rooms) {
      ...
      for (const f of uniqueFiles) {
        ...
        if (tracked.rows.length > 0) {
          ...
          if (newSize !== oldSize) {
            // 更新 recording_files 表的文件大小
            await pool.query(
              `UPDATE recording_files SET file_size = $1, checked_at = NOW() WHERE id = $2`,
              [newSize, fileId]
            );

            // 更新 recordings 表的文件大小
            await pool.query(
            ...
            if (sizeDiff > 0) {
              await pool.query(
                `UPDATE recording_sessions SET total_size = total_size + $1 WHERE id = $2`,
                [sizeDiff, room.session_id]
              );
            }
          }
          continue;

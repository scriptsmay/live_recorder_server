...
async function extract(record) {
  ...
  if (!record.play_url) return { success: false, error: '缺少 play_url，无法提取 m3u8' };
  return {
    success: false,
    error: 'm3u8 提取客户端尚未接入；请先通过历史迁移或后续 API 客户端写入 m3u8_url',
  };
}
...
async function download(record) {
  ...
  const result = await runCommand('yt-dlp', ['-o', outputPath, record.m3u8_url], { cwd: workDir });
  if (!result.success) return result;
  const stat = fs.statSync(outputPath);
  return { success: true, rawFilePath: outputPath, fileSize: stat.size, output: result.output };
}
...
async function fix(record) {
  const files = JSON.parse(record.cut_file_paths || '[]');

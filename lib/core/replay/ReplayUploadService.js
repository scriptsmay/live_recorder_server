...
function getReplayTemplateVars(record) {
  ...
  const baseVars = UploadService.getTemplateVars(
    { room_name: record.principal_name, room_url: record.play_url },
  ...
  return {
    ...baseVars,
    principal_name: record.principal_name || record.principal_id || 'unknown',
...
class ReplayUploadService {
  static async executeUpload(replayRecordId) {
    ...
    notify.uploadStart(record.principal_name, tmpl.name, files.length, record.play_url);
  ...
  static async _runUpload(uploadRecordId, record, tmpl, files, title, desc, tags, source) {
    ...
    try {
      ...
      if (result.success) {
        ...
        await pool.query(
          `UPDATE replay_records SET status='uploaded', bv_id=$1, uploaded_at=NOW(), updated_at=NOW() WHERE id=$2`,
          [result.bvId, record.id]
        );
        notify.uploadComplete(record.principal_name, title, result.bvId, record.play_url);
        ...
        const postResult = await afterUpload(
          ...
          `replay_${record.id}`,
          tmpl.name,
          `replay_${uploadRecordId}`,
          record.principal_name,
          record.play_url
        );
        if (postResult) {
          await pool.query(`UPDATE replay_upload_records SET output = output || $1 WHERE id = $2`, [
        ...
        await notify.uploadFailed(record.principal_name, tmpl.name, title, result.error, record.play_url);
      }
    } catch (err) {
      await pool
        .query(

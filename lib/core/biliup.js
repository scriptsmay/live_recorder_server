...
class Biliup {
  ...
  async upload({
    ...
    const UPLOAD_TIMEOUT_MS = 30 * 60 * 1000; // 30 分钟超时
    ...
    return new Promise((resolve) => {
      ...
      // 记录进程启动信息
      const startupInfo = `[biliup] pid=${proc.pid} cwd=${uploadCwd} path=${biliupPath}`;
      ...
      const timer = setTimeout(() => {
        if (!settled) {
          ...
          procLog.destroy();
          const timeoutMsg = `上传超时（${UPLOAD_TIMEOUT_MS / 60000}分钟），已强制终止进程`;
          logStream.write(`\n[ERROR] ${timeoutMsg}\n`);
          resolve({
            success: false,
            error: timeoutMsg,
      ...
      proc.on('error', (err) => {
        if (!settled) {
          ...
          clearTimeout(timer);
          procLog.destroy();
          const errMsg = `进程启动失败: ${err.message}`;
          logStream.write(`\n[ERROR] ${errMsg}\n`);
          resolve({
            success: false,
            error: errMsg,

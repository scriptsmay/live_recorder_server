...
class TranscodeQueue {
  ...
  async enqueue(taskData) {
    try {
      ...
      console.log(`[转码队列] 任务入队: ${path.basename(taskData.videoPathToTrans)}`);
      this.processQueue();
    } catch (err) {
  ...
  async processTask(task) {
    const { videoPathToTrans, mp4Path, sessionId: _sessionId } = task;
  ...
  async triggerHLSGeneration(mp4Path, _sessionId) {
    try {
      ...
      const result = await pool.query(
        `SELECT id FROM recordings WHERE file_path = $1 LIMIT 1`,
        [mp4Path]
      );
      ...
      if (result.rows.length > 0) {
        ...
        const genResult = await hlsGenerator.generateForRecording(recordingId, 'recording');

...
class TranscodeQueue {
  ...
  async createTranscodeRecord(taskData) {
    try {
      await pool.query(
        ...
        [taskData.sessionId, taskData.flvPath, taskData.mp4Path]
      );
    } catch (err) {
   ...
   * 更新转码记录为进行中
   */
  async startTranscodeRecord(flvPath) {
    try {
      await pool.query(
  ...
  async processTask(task) {
    ...
    try {
      await this.startTranscodeRecord(flvPath);
      ...
      if (autoTranscode !== 'true') {
        ...
        await this.completeTranscodeRecord(flvPath, mp4Path, false);
        return;
      }
      ...
      if (stat.size < thresholdBytes) {
        console.log(
          `[转码队列] 文件小于碎片阈值，跳过: ${path.basename(flvPath)} (${(stat.size / 1024 / 1024).toFixed(1)}MB < ${thresholdValue}MB)`
        );
        await this.completeTranscodeRecord(flvPath, mp4Path, false);
        return;
      }
      ...
      const result = await transcoder.fastTranscode(flvPath, mp4Path, task.sessionId);
      ...
      if (result.success) {
        ...
        await this.completeTranscodeRecord(flvPath, mp4Path, true);

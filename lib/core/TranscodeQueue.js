...
class TranscodeQueue {
  ...
  async processTask(task) {
    ...
    try {
      ...
      if (result.success) {
        ...
        // 自动弹幕压制（如果启用且有 ASS 文件）
        this.triggerDanmakuBurn(mp4Path, _sessionId).catch((err) => {
          console.warn('[转码队列] 弹幕压制触发失败:', err.message);
        });
  ...
  /**
   * 触发弹幕压制（如果启用且有 ASS 文件）
   */
  async triggerDanmakuBurn(mp4Path, _sessionId) {
    try {
      const autoBurn = await DataService.getSetting('auto_burn_danmaku', 'false');
      if (autoBurn !== 'true') return;

      const result = await pool.query(`SELECT id, danmaku_ass_path FROM recording_files WHERE file_path = $1 LIMIT 1`, [
        mp4Path,
      ]);
      ...
      if (!recording.danmaku_ass_path || !fs.existsSync(recording.danmaku_ass_path)) {
        return; // 无 ASS 文件，不触发
      }

      const ext = path.extname(mp4Path);
      const outputPath = mp4Path.replace(new RegExp(`${ext}$`), '_danmaku.mp4');

      console.log(`[转码队列] 触发弹幕压制: ${path.basename(mp4Path)}`);
      ...
      await danmakuBurnQueue.enqueue({
        ...
        segmentIndex: 0,
        inputPath: mp4Path,
        assPath: recording.danmaku_ass_path,
        outputPath,
        force: false,
      });
    } catch (err) {
      console.warn('[转码队列] 触发弹幕压制异常:', err.message);
    }
  }

...
function getNasConfig() {
  ...
  if (!host || !user || !dir) {
    throw new Error(
      'NAS 配置不完整，请设置 NAS_HOST、NAS_USER、NAS_BACKUP_DIR'
    );
  }
...
async function backupToNAS(files, logKey) {
  ...
  const {
    ...
    logPath,
    logCommand,
  } = createProcLog('nas_backup', logKey);
...
async function deleteLocalFiles(files, logKey, sessionId) {
  ...
  if (sessionId) {
    try {
      ...
      logStream.write(
        `[DB会话更新失败] session ${sessionId}: ${dbErr.message}\n`
      );
    }
  }
  logStream.write(
    `[文件清理] 完成: 删除 ${results.deleted}, 失败 ${results.failed}\n`
  );

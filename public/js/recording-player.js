...
const RecordingPlayer = {
   ...
   * @param {boolean} [opts.danmaku=false] - 是否播放弹幕压制版
   * @returns {Promise<void>}
   */
  async play(recordingId, filename, ext, toastFn = toast, opts = {}) {
    const title = filename || '视频预览';
    const danmakuSuffix = opts.danmaku ? '?type=danmaku' : '';
    ...
    try {
      ...
      const src = `/api/recordings/${recordingId}/stream${danmakuSuffix}`;
      const res = await fetch(src, { headers: { Range: 'bytes=0-1' } });
      if (res.ok || res.status === 206) {
        openVideoPlayer(src, opts.danmaku ? title + ' (弹幕版)' : title, ext);
      } else {
  ...
  init(toastFn = toast) {
    document.addEventListener('click', async (e) => {
      ...
      const danmaku = btn.dataset.danmaku === 'true';

      await this.play(recordingId, filename, ext, toastFn, { danmaku });
    });
  },
};

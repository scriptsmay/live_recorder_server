...
class DanmakuBurner {
  constructor() {
    ...
    this._capabilities = null; // 缓存，避免每次压制都 fork ffmpeg
  }

  /**
   * 获取 FFmpeg 能力（首次调用后缓存）
   */
  async _getCapabilities() {
    if (!this._capabilities) {
      this._capabilities = await this.probeCapabilities();
    }
   ...
   * @param {number} [params.segmentIndex] - 分段序号（用于日志文件命名）
  ...
  async burn(params) {
    ...
    if (!force && fs.existsSync(outputPath)) {
      return {
        ...
        outputPath,
        duration: 0,
        error: `输出文件已存在（使用 force=true 覆盖）`,
        logPath: null,
      };
    }
    ...
    // 日志命名：dmk_{sessionId}_seg{index}_{inputBasename}.log
    const inputBasename = path.basename(inputPath, path.extname(inputPath));
    let logId = null;
    if (sessionId != null) {
      const segPart = segmentIndex != null ? `_seg${segmentIndex}` : '';
      logId = `${sessionId}${segPart}_${inputBasename}`;
    }
    const procLog = createProcLog('dmk', logId);

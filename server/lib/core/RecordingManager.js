...
class RecordingManager {
  ...
  async updateSessionCover(sessionId, coverPath) {
    await pool.query(`UPDATE recording_sessions SET cover_path = $1 WHERE id = $2`, [coverPath, sessionId]);
  }
   ...
   * @returns
   */
  async createSession({ room, streamUrl, sessionId, outputPath, caption, reuseSession, resumeCount, coverUrl }) {
    ...
    if (!finalSessionId) {
      ...
      const session = await pool.query(
        `INSERT INTO recording_sessions (room_url, started_at, output_dir, status, caption, stream_url, cover_url)

...
async function createSession(token, username) {
  ...
  const r = getRedis();
  await r.set(`auth:session:${token}`, JSON.stringify({ username, createdAt: Date.now() }), { EX: ttl });
  return ttl;
}

async function destroySession(token) {
  await getRedis().del(`auth:session:${token}`);
}
...
async function clearFailures(ip) {
  await getRedis().del(`auth:fail:${ip}`);
}
...
async function ensureAdminCredentials() {
  ...
  if (count === 0) {
    ...
    // 3. 写库
    await pool.query('INSERT INTO admin_users (username, password_hash) VALUES ($1, $2)', [defaultUsername, hash]);

...
以下两处问题**已在本地代码修复，但尚未发版部署**（线上仍跑 `v1.7.16`）：
...
> 部署新版本后：问题 A 的 `mtime` 在扫描阶段即写入（存量数据首轮扫描也会补全），次日定时任务即可正常清理；问题 B 的孤儿 HLS 目录即可进入 `deletable` 并被 `HLSCleanupService` 以 `success_noop` 清理。
